            ...
            if group == "ANY":
                ...
                if group in groups.keys():
                    if groups[group]["object type"] == "IPAddressExpression":
                        ...
                        dfw_rules[str(cmap["display_name"])]["rules"][str(rule["sequence_number"])]["Source(s)"][group].append(str(group+"-cannot process group, check contents in GUI, may be empty"))            
                else:
            ...
            if group == "ANY":
                ...
                if group in groups.keys():
                    if groups[group]["object type"] == "IPAddressExpression":
                        ...
                        dfw_rules[str(cmap["display_name"])]["rules"][str(rule["sequence_number"])]["Destination(s)"][group].append(str(group+"-cannot process group, check contents in GUI, may be empty"))
                else:
                    print("??????????????????cant find this group1?????????????????")
                    print(group)
                    print(str(cmap["display_name"])+":"+str(rule["sequence_number"]))
