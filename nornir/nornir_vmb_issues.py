...
# Variables
config_file = "/dbdev/nornir/config_files/vmb_config.yaml"
...
def latency_check():
    ...
    nr_devices = nr.filter(role="iosxr")



    def gatherfacts(task:Task,netmiko_bar) -> Result:
        task.run(task=netmiko_send_command, command_string="ping 10.92.10.40 source TenGigE0/1/0/10.1 rep 500 size 500", read_timeout=100,use_genie=False, use_timing=False)
        task.run(task=netmiko_send_command, command_string="ping 10.92.10.1 source TenGigE0/1/0/10.1 rep 500 size 500", read_timeout=100,use_genie=False, use_timing=False)
    ...
    latency = hf.parse_vmb_pings(device_facts,1,2)
    filepath = '/dbdev/nornir/vmb_latency.json'
...
if __name__ == "__main__":
    latency_check()
    schedule.every(5).minutes.do(latency_check)
    while True:
