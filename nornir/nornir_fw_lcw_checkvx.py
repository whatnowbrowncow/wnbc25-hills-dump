...
if __name__ == "__main__":
    def get_subnet_config(task):
        for device,details in contexts.items():
            if device == str(task.host):
                print('############################'+device+' matches '+str(task.host)+'#############################')
                for context in details:
                    #print(context.result)
                    for con,deeta in context.result.items():
                        cmd1 = "changeto context "+con
                        cmd2 = "show run | inc "+route
                        cmd3 = "show access-list | inc "+route
                        multi_result1= task.run(task=netmiko_send_command, command_string=cmd1, use_genie=False, use_timing=True)
                        multi_result2= task.run(task=netmiko_send_command, command_string=cmd2, use_genie=False, use_timing=True)
                        multi_result3= task.run(task=netmiko_send_command, command_string=cmd3, use_genie=False, use_timing=True)
                        #print('Multiresult1####################################')
                        #print(multi_result1[0])
                        #print('Multiresult2####################################')
                        #print(multi_result2[0])
                        #print('Multiresult3####################################')
                        #print(multi_result3[0])

                        #csystem = nr_devices.run(task=netmiko_send_command, command_string="changeto context "+con)
                        #print_result(csystem)
                        #ccontexts = nr_devices.run(task=netmiko_send_command, command_string="show run | inc "+route, use_genie=False, use_timing=True)
                        #print_result(ccontexts)
                        #print('result-----------------')
                        #print(ccontexts)
                        #print(ccontexts[device].result)
                        report[device]['contexts'][con]={}
                        report[device]['contexts'][con]['Config']=multi_result2[0].result
                        report[device]['contexts'][con]['acls']=multi_result3[0].result
                        print(con+' complete')
            else:
                print('############################'+device+' DOES NOT MATCH '+str(task.host)+'#############################')
    ...
    for device,details in contexts.items():
        report[device]={}
        report[device]['contexts']={}
        print(device)
        #print(details.result)
        print('contexts--------')
        for k,v in details.result.items():
            
            print(k)
    print('Calling task function START ++++++++++++++++++++++++++++++++++++++++')
    acls = nr_devices.run(task=get_subnet_config)
    print('Calling task function END ------------------------------------------')
    #print_result(acls)
        #for context in details:
        #    #print(context.result)
        #    
        ...
        #        print(ccontexts)
        #        print(ccontexts[device].result)
        #        report[device]['contexts'][con]={ccontexts[device].result}
        #        
                
    pprint.pprint(report)
    
    filepath = './lcw_report.json'
