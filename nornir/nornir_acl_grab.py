...
if __name__ == "__main__":
    ...
    nr_devices = nr.filter(F(role="ios") | F(role="oob"))
    ...
    print("obtaining ACLs, this may take a moment..........")

 
    
    acls = nr_devices.run(task=netmiko_send_command, command_string="show access-lists", use_genie=True, use_timing=True)
    #print(acls)
    #print(type(acls))
    #print(dir(acls))
        ...
        try:
            for acl,rules in details[0].result.items():
                ...
                if 'aces' in details[0].result[acl]:
                    ...
                    device_acls[device][acl]['type']=details[0].result[acl]['acl_type']
                            ...
                            if 'destination_network' in details[0].result[acl]['aces'][rule]['matches']['l3']['ipv4'].keys():
                                for k,v in details[0].result[acl]['aces'][rule]['matches']['l3']['ipv4']['source_network'].items():
                                    for k1,v1 in details[0].result[acl]['aces'][rule]['matches']['l3']['ipv4']['destination_network'].items():
                                        device_acls[device][acl]['rules'][rule]=str(details[0].result[acl]['aces'][rule]['actions']['forwarding'])+' '+str(re.sub(' 0.0.0.0','',k))+' '+str(re.sub(' 0.0.0.0','',k1))
                            else:
                                for k,v in details[0].result[acl]['aces'][rule]['matches']['l3']['ipv4']['source_network'].items():
                                    device_acls[device][acl]['rules'][rule]=str(details[0].result[acl]['aces'][rule]['actions']['forwarding'])+' '+str(re.sub(' 0.0.0.0','',k))
                                    #print('hello')
                                    #print(k)
                                    #print(v)
