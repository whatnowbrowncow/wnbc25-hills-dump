...
if __name__ == "__main__":
    ...
    def tunnel12_shutdown_config(task:Task,netmiko_bar) -> Result:
        task.run(task=netmiko_send_config, config_commands=["interface tunnel 12","shutdown"])
        netmiko_bar.update()
    def tunnel12_no_shutdown_config(task:Task,netmiko_bar) -> Result:
        task.run(task=netmiko_send_config, config_commands=["interface tunnel 12","no shutdown"])
        netmiko_bar.update()
    
    console.print("[blue]##################\nStep 2 of 4 - shutting down tunnel 12 ({} devices), this may take a while\n##################".format(len(nr_spokes.inventory.hosts)))
    ...
    with tqdm(
        total=len(cfg_devices.inventory.hosts), desc="progress",
            ...
            # we call our grouped task passing both bars
            tunnel12_shut_facts=nr_spokes.run(
                task=tunnel12_shutdown_config,
                netmiko_bar=netmiko_bar,
    ...
    tunnel12_shut_facts_clean,tunnel12_shut_facts_failed_hosts = rhf.clean_facts(tunnel12_shut_facts)

    


    #print('####################testing function#####################')
    tunnel12_shut_processed_results,tunnel12_shut_full_processed_results = rhf.process_update_results(tunnel12_shut_facts_clean)
    #print('#########################################')
    #time.sleep(15)
    console.print("[blue]##################\nStep 3 of 4 - bringing up tunnel 12 ({} devices), this may take a while\n##################".format(len(nr_spokes.inventory.hosts)))
    ...
    with tqdm(
        total=len(cfg_devices.inventory.hosts), desc="progress",
            ...
            # we call our grouped task passing both bars
            tunnel12_noshut_facts=nr_spokes.run(
                task=tunnel12_no_shutdown_config,
                netmiko_bar=netmiko_bar,
    ...
    #clean config results
    tunnel12_noshut_facts_clean,tunnel12_noshut_facts_failed_hosts = rhf.clean_facts(tunnel12_noshut_facts)

    


    #print('####################testing function#####################')
    tunnel12_noshut_processed_results,tunnel12_noshut_full_processed_results = rhf.process_update_results(tunnel12_noshut_facts_clean)
    #print('#########################################')
    ...
    if len(tunnel12_shut_facts_failed_hosts.items()) > 0:
        console.print("[bold italic red]The following devices have failed at stage 2:")
        for device,reason in tunnel12_shut_facts_failed_hosts.items():
            console.print('[red]{}[/red][bold red]:{}'.format(device,reason))
    if len(tunnel12_noshut_facts_failed_hosts.items()) > 0:
        console.print("[bold italic red]The following devices have failed at stage 3:")
        for device,reason in tunnel12_noshut_facts_failed_hosts.items():
            console.print('[red]{}[/red][bold red]:{}'.format(device,reason))
