# variables.tf - Production Environment Configuration
# Environment-specific variable overrides for prod in eu-west-1
# These values override module defaults and configure production-specific settings
...
variable "local_supernet_cidr" {
  description = "Production region supernet CIDR block"
  type        = string
...
variable "environment_ip_rules" {
  description = "Production-specific IP-based firewall rules"
  type = list(object({
    name        = string
...
variable "environment_domain_rules" {
  description = "Production-specific domain allowlist rules"
  type = object({
...
variable "enable_test_instances" {
  description = "Deploy test instances for network validation in production"
  type        = bool
  default     = true
}

variable "test_instance_type" {
  description = "EC2 instance type for production test instances"
  type        = string
  default     = "t3.micro"
}
...
variable "enable_peering" {
  description = "Enable Transit Gateway peering (prod accepts connections)"
  type        = bool
  default     = true
}

variable "is_peering_initiator" {
  description = "Whether this environment initiates peering (false for prod - accepter role)"
  type        = bool
  default     = false
}

# ------------------------------------------------------
# Production Environment Settings (Self-Reference)
# ------------------------------------------------------
...
#  description = "Production Transit Gateway ID"
#  type        = string
#  default     = "tgw-0490ce184eb6abde8"
#}
...
# ------------------------------------------------------
# Peer Environment Settings (For Return Routing)
# ------------------------------------------------------

variable "nonprod_account_id" {
  description = "Non-production account ID (for reference)"
  type        = string
  default     = ""
}

variable "nonprod_tgw_id" {
  description = "Non-production Transit Gateway ID (for reference)"
  type        = string
  default     = ""
}
