...
def print_selection(summarytable):
    ...
    if choice == '1':
        ...
        if stacktable == 0:
            console.print('''[red]\n###################################################\nNo switch information available for either device\n###################################################\n''')
        else:    
        ...
        if ethertable == 0:
            console.print('''[red]\n###################################################\nNo etherchannel information available for either device\n###################################################\n''')
        else:
...
with open('./json_log_files/'+device1+'/master_dict_curr_latest.json') as json_file: 
    master_dict_pre=json.load(json_file)

with open('./json_log_files/'+device2+'/master_dict_curr_latest.json') as json_file:
    master_dict_post=json.load(json_file)
master_dict_pre.update(master_dict_post)
master_dict = master_dict_pre

with open('./json_log_files/temp/master_dict_merge_latest.json', "w") as outfile: 
    json.dump(master_dict, outfile)

#with open('./json_log_files/'+target_device+'/master_dict_merge_'+curtime+'.json', "w") as outfile: 
#    json.dump(master_dict, outfile)

devicepre = str(device1) + '-curr'
devicepost = str(device2) + '-curr'
...
inttable,missingint,missmatchfound = compare_interfaces('temp',devicepre,devicepost)
mactable,macmatch = compare_mac('temp',devicepre,devicepost)
vlansumtable,vlantable,vlanmismatch = compare_vlan('temp',devicepre,devicepost)
routingtablepre,routingtablepost,allroutesmatch = compare_routing('temp',devicepre,devicepost)
if master_dict[devicepre]['os'] == master_dict[devicepost]['os']:
    vertable,vermismatch,postverhigh = compare_version('temp',devicepre,devicepost,upgrade)
    osmatch = True
else:
    console.print('''\n[dim red]OS versions don't match pre/post, skipping version check...........\n''')
    osmatch = False
stacktable,switchmismatch,stackmismatch = compare_switch('temp',devicepre,devicepost)
ethertable,ethermismatchfound,ethermembermismatchfound = compare_etherchannel('temp',devicepre,devicepost)
vrf_routing_results,vrfneighbormismatch = compare_vrfrouting('temp',devicepre,devicepost)
