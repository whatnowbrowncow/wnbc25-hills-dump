...
def get_epg_data(raw_input,data_position = 0):
    epg_data = {}
    epg_data['Tenants'] = {}
    multiple_encaps = []
    for hostname, entry_1_level in raw_input.items():
        try:
            if entry_1_level[0].failed == False:
                epgs = re.findall('(Tenant(?:.*\n)+?)(?=Tenant|\Z)',str(entry_1_level[data_position].result))
                
                for epg in epgs:
                    encaps = []
                    tenant = re.search('Tenant\s+:\s(\S+)',str(epg)).group(1)
                    application = re.search('Application\s+:\s(\S+)',str(epg)).group(1)
                    AEPg = re.search('AEPg\s+:\s(\S+)',str(epg)).group(1)
                    endpoints = re.findall('((?:[0-9A-Fa-f]{2}[:-]){5}(?:[0-9A-Fa-f]{2}).*)',epg)
                    #try:
                    #    fw_encap = re.search('(?:[0-9A-Fa-f]{2}[:-]){5}(?:[0-9A-Fa-f]{2}).*FW01-Data_PolGrp.*vlan-(\d+)',epg).group(1)
                    #    #fw_encap = re.search('(?:[0-9A-Fa-f]{2}[:-]){5}(?:[0-9A-Fa-f]{2}).*uk-ld6-fw05-data_polgrp.*vlan-(\d+)',epg).group(1)
                    #except:
                    #    try:
                    #        fw_encap = re.search('(?:[0-9A-Fa-f]{2}[:-]){5}(?:[0-9A-Fa-f]{2}).*fw02-pri-ha_polgrp.*vlan-(\d+)',epg).group(1)
                    #        #fw_encap = re.search('(?:[0-9A-Fa-f]{2}[:-]){5}(?:[0-9A-Fa-f]{2}).*uk-ld6-fw05-data_polgrp.*vlan-(\d+)',epg).group(1)
                    #    except:
                    #        try:
                    #            fw_encap = re.search('(?:[0-9A-Fa-f]{2}[:-]){5}(?:[0-9A-Fa-f]{2}).*fw03-pri-ha_polgrp.*vlan-(\d+)',epg).group(1)
                    #            #fw_encap = re.search('(?:[0-9A-Fa-f]{2}[:-]){5}(?:[0-9A-Fa-f]{2}).*uk-ld6-fw05-data_polgrp.*vlan-(\d+)',epg).group(1)
                    #        except:
                    #            try:
                    #                fw_encap = re.search('(?:[0-9A-Fa-f]{2}[:-]){5}(?:[0-9A-Fa-f]{2}).*fw01-pri-ccl_polgrp.*vlan-(\d+)',epg).group(1)
                    #                #fw_encap = re.search('(?:[0-9A-Fa-f]{2}[:-]){5}(?:[0-9A-Fa-f]{2}).*uk-ld6-fw05-data_polgrp.*vlan-(\d+)',epg).group(1)
                    #            except:
                    try:
                        fw_encap = re.search('(?:[0-9A-Fa-f]{2}[:-]){5}(?:[0-9A-Fa-f]{2}).*101 102.*vlan-(\d+)',epg).group(1)
                        #fw_encap = re.search('(?:[0-9A-Fa-f]{2}[:-]){5}(?:[0-9A-Fa-f]{2}).*uk-ld6-fw05-data_polgrp.*vlan-(\d+)',epg).group(1)
                    except:
                        try:
                            fw_encap = re.search('(?:[0-9A-Fa-f]{2}[:-]){5}(?:[0-9A-Fa-f]{2}).*vlan-(1[03-9]\d+)',epg).group(1)
                            #fw_encap = re.search('(?:[0-9A-Fa-f]{2}[:-]){5}(?:[0-9A-Fa-f]{2}).*uk-ld6-fw05-data_polgrp.*vlan-(\d+)',epg).group(1)
                        except:
                            try:
                                fw_encap = re.search('(?:[0-9A-Fa-f]{2}[:-]){5}(?:[0-9A-Fa-f]{2}).*vlan-(\d{3}) ',epg).group(1)
                            except:
                                try:
                                    fw_encap = re.search('(?:[0-9A-Fa-f]{2}[:-]){5}(?:[0-9A-Fa-f]{2}).*vlan-([2-9]\d+) ',epg).group(1)
                                except:
                                    try:
                                        fw_encap = re.search('(?:[0-9A-Fa-f]{2}[:-]){5}(?:[0-9A-Fa-f]{2}).*vlan-(\d+) ',epg).group(1)
                                    except:
                                        fw_encap="FW Encap VLAN not found"
                    #try:
                    #    fw2_encap = re.search('(?:[0-9A-Fa-f]{2}[:-]){5}(?:[0-9A-Fa-f]{2}).*FW01-Data_PolGrp.*vlan-(\d+)',epg).group(1)
                    #    #fw_encap = re.search('(?:[0-9A-Fa-f]{2}[:-]){5}(?:[0-9A-Fa-f]{2}).*uk-ld6-fw05-data_polgrp.*vlan-(\d+)',epg).group(1)
                    #except:
                    #    fw2_encap="FW Encap VLAN not found"
                    if tenant not in epg_data['Tenants'].keys():
                        epg_data['Tenants'][tenant]={}
                        epg_data['Tenants'][tenant]['Application_profiles'] = {}
                    if application not in epg_data['Tenants'][tenant]['Application_profiles'].keys():
                        epg_data['Tenants'][tenant]['Application_profiles'][application]={}
                        epg_data['Tenants'][tenant]['Application_profiles'][application]['EPGs'] = {}
                    if AEPg not in epg_data['Tenants'][tenant]['Application_profiles'][application]['EPGs'].keys():
                        epg_data['Tenants'][tenant]['Application_profiles'][application]['EPGs'][AEPg]={}
                        epg_data['Tenants'][tenant]['Application_profiles'][application]['EPGs'][AEPg]['FW_encap_vlan']=fw_encap
                        epg_data['Tenants'][tenant]['Application_profiles'][application]['EPGs'][AEPg]['Endpoints']={}
                    for endpoint in endpoints:
                        endpoint_mac = re.search('((?:[0-9A-Fa-f]{2}[:-]){5}(?:[0-9A-Fa-f]{2}))',endpoint).group(1)
                        encap = re.search('(vlan-\S+)',endpoint).group(1)
                        epg_data['Tenants'][tenant]['Application_profiles'][application]['EPGs'][AEPg]['Endpoints'][endpoint_mac]=encap
                        encaps.append(encap)
                        encaplist = Counter(encaps)
                    if len(encaplist.keys()) > 1:
                        encapvlans=[]
                        for vlan,count in encaplist.items():
                            encapvlans.append('{}({})'.format(vlan,count))
                        multiple_encaps.append([AEPg,tenant,application,' | '.join(encapvlans)])
    ...
    return epg_data,multiple_encaps
