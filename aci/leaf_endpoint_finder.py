...
if __name__ == "__main__":
    ...
    nr_devices = nr.filter(role="leaf")

    def gatherfacts(task:Task,netmiko_bar) -> Result:
        task.run(task=netmiko_send_command, command_string="show system internal epm endpoint mac {}".format(endpoint), use_genie=True, use_timing=False, read_timeout=20)
    ...
    for device,results in device_facts.items():
         print("{}:{}".format(device,results[1]))
    exit()
  #crypto_config = rhf.parse_crypto(device_facts,1,2)
    epg_data,multiple_endpoints = ahf.get_epg_data(device_facts,1)
    #print(epg_data)

    filepath1 = '/dbdev/aci/outputs/endpoints.json'
