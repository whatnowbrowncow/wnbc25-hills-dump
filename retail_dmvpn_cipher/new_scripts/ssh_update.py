...
def ssh_check():
    ...
    with tqdm(
        ...
        ssh_update=cfg_devices.run(task=update_cypher_config,netmiko_bar=netmiko_bar)
    ...
    filepath7 = '/dbdev/retail_dmvpn_cipher/outputs/results/ssh/'+curfoltime+'/processed_results.json'
    with open(filepath7, "w") as outfile: 
        json.dump(processed_results, outfile)
    #save it
    with open(f'/dbdev/retail_dmvpn_cipher/outputs/results/ssh/'+curfoltime+'/full_processed_results.pickle', 'wb') as file:
