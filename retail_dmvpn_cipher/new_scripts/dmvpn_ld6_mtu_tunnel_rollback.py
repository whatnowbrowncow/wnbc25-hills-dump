...
if __name__ == "__main__":
    ...
    for device, data in combined_data['tunnel 12 pre-change'].items():
        ...
        if combined_data['tunnel 12 pre-change'][device]['mtu'] == old_mtu:
            combined_data['tunnel 12 pre-change'][device]['mtu_match'] = '[green]\u2713 Old MTU[/green]'
            mtu_match = '[green]\u2713 Old MTU[/green]'
        else:
            combined_data['tunnel 12 pre-change'][device]['mtu_match'] = '[red]\u2717 New MTU[/red]'
    ...
    with open('/dbdev/retail_dmvpn_cipher/mtu_tunnel_rollback_ld6.txt') as f:
    ...
    for device, data in combined_data['tunnel 12 post-change'].items():
        ...
        if combined_data['tunnel 12 post-change'][device]['mtu'] == old_mtu:
            combined_data['tunnel 12 post-change'][device]['mtu_match'] = '[green]\u2713 Old MTU[/green]'
            mtu_match = '[green]\u2713 Old MTU[/green]'
        else:
            combined_data['tunnel 12 post-change'][device]['mtu_match'] = '[red]\u2717 New MTU[/red]'
            mtu_match = '[red]\u2717 New MTU[/red]'
            update_required = True
