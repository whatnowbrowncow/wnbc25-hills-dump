...
import dmvpn_pre_change_tunnel_check as tc
...
dmvpnmtupretable = Table(title='DMVPN LD6 Tunnel 12 + MTU Pre change Summary',show_header=True, header_style="bold blue")
dmvpnmtupretable.add_column('Device',justify='center')
dmvpnmtupretable.add_column('Tunnel 12 UP',justify='center')
...
dmvpnmtuposttable = Table(title='DMVPN LD6 Tunnel 12 + MTU Post change Summary',show_header=True, header_style="bold blue")
dmvpnmtuposttable.add_column('Device',justify='center')
dmvpnmtuposttable.add_column('Tunnel 12 UP',justify='center')
dmvpnmtuposttable.add_column('EIGRP  neighbour 10.92.27.1',justify='center')
...
dmvpnmturesultstable = Table(title='DMVPN LD6 Tunnel 12 + MTU change results Summary',show_header=True, header_style="bold blue")
dmvpnmturesultstable.add_column('Device',justify='center')
dmvpnmturesultstable.add_column('Tunnel 12 UP pre/post',justify='center')
dmvpnmturesultstable.add_column('EIGRP nbr 10.92.27.1 up pre/post',justify='center')
...
if __name__ == "__main__":
    ...
    for device, data in device_facts.items():
        try:
            ...
            combined_data['tunnel 12 pre-change'][device]['mtu'] = spoke_tunnels_parsed[device]['Tunnel12']['mtu']
    ...
    for device, data in combined_data['tunnel 12 pre-change'].items():
        ...
        if combined_data['tunnel 12 pre-change'][device]['mtu'] == new_mtu:
            combined_data['tunnel 12 pre-change'][device]['mtu_match'] = '[green]\u2713 New MTU[/green]'
            mtu_match = '[green]\u2713 New MTU[/green]'
        else:
            combined_data['tunnel 12 pre-change'][device]['mtu_match'] = '[red]\u2717 old MTU[/red]'
    ...
    with open('/dbdev/retail_dmvpn_cipher/mtu_tunnel_config_ld6.txt') as f:
    ...
    for device, data in combined_data['tunnel 12 post-change'].items():
        ...
        if combined_data['tunnel 12 post-change'][device]['psk'] == new_psk:
            ...
            psk_match = '[red]\u2717 old PSK[/red]'
            update_required = True
        if combined_data['tunnel 12 post-change'][device]['mtu'] == new_mtu:
            combined_data['tunnel 12 post-change'][device]['mtu_match'] = '[green]\u2713 New MTU[/green]'
            mtu_match = '[green]\u2713 New MTU[/green]'
        else:
            combined_data['tunnel 12 post-change'][device]['mtu_match'] = '[red]\u2717 old MTU[/red]'
            mtu_match = '[red]\u2717 old MTU[/red]'
            update_required = True
        ...
        if update_required == True:
            ...
            update = '[yellow]\u2717[/yellow]'
            devices_configured.append(device)
        dmvpnmtuposttable.add_row(device,tu12state,eigrp_neighbour,profile_match,psk_match,mtu_match,update)
