...
if __name__ == "__main__":
    ...
    with open('/dbdev/retail_dmvpn_cipher/psk_mtu_tunnel_config_scc.txt') as f:
    ...
    for device, data in combined_data['tunnel 11 post-change'].items():
        ...
        if combined_data['tunnel 11 post-change'][device]['psk'] == new_psk:
            ...
            psk_match = '[red]\u2717 old PSK[/red]'
            update_required = True
        if combined_data['tunnel 11 post-change'][device]['mtu'] == new_mtu:
            combined_data['tunnel 11 post-change'][device]['mtu_match'] = '[green]\u2713 New MTU[/green]'
            mtu_match = '[green]\u2713 New MTU[/green]'
        else:
            combined_data['tunnel 11 post-change'][device]['mtu_match'] = '[red]\u2717 old MTU[/red]'
            mtu_match = '[red]\u2717 old MTU[/red]'
            update_required = True
