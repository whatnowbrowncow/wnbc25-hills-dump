...
if __name__ == "__main__":
    ...
    def gatherfacts(task:Task,netmiko_bar) -> Result:
        task.run(task=netmiko_send_command, command_string="show run all | section crypto", use_genie=False, use_timing=True)
        task.run(task=netmiko_send_command, command_string="show run | section interface Tunnel", use_genie=False, use_timing=True)
        task.run(task=netmiko_send_command, command_string="show ip eigrp neighbors", use_genie=True, use_timing=True)
    ...
    crypto_config = rhf.parse_crypto(device_facts,1)
    spoke_tunnels_parsed = rhf.get_tunnel_interface_data(device_facts,2)
    eigrp_neighbours = rhf.spoke_eigrp_neighbours(device_facts,3)
