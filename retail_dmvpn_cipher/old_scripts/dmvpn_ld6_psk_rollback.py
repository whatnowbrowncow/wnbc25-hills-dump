...
dmvpnpskpretable = Table(title='DMVPN LD6 Tunnel 12 + PSK Pre change Summary',show_header=True, header_style="bold blue")
dmvpnpskpretable.add_column('Device',justify='center')
dmvpnpskpretable.add_column('Tunnel 12 UP',justify='center')
dmvpnpskpretable.add_column('EIGRP  neighbour 10.92.27.1',justify='center')
dmvpnpskpretable.add_column('Profile being used',justify='center')
dmvpnpskpretable.add_column('PSK old/new',justify='center')
dmvpnpskpretable.add_column('Update required',justify='center')

dmvpnpskposttable = Table(title='DMVPN LD6 Tunnel 12 + PSK Post change Summary',show_header=True, header_style="bold blue")
dmvpnpskposttable.add_column('Device',justify='center')
dmvpnpskposttable.add_column('Tunnel 12 UP',justify='center')
dmvpnpskposttable.add_column('EIGRP  neighbour 10.92.27.1',justify='center')
dmvpnpskposttable.add_column('Profile being used',justify='center')
dmvpnpskposttable.add_column('PSK old/new',justify='center')
dmvpnpskposttable.add_column('Update required',justify='center')

dmvpnpskresultstable = Table(title='DMVPN LD6 Tunnel 12 + PSK change results Summary',show_header=True, header_style="bold blue")
dmvpnpskresultstable.add_column('Device',justify='center')
dmvpnpskresultstable.add_column('Tunnel 12 UP pre/post',justify='center')
dmvpnpskresultstable.add_column('EIGRP nbr 10.92.27.1 up pre/post',justify='center')
dmvpnpskresultstable.add_column('Profile pre/post',justify='center')
dmvpnpskresultstable.add_column('PSK pre/post',justify='center')
...
dmvpnposttable.add_column('Tunnel 12 state match',justify='center')
dmvpnposttable.add_column('Tu12 EIGRP  neighbour match',justify='center')
...
if __name__ == "__main__":
    ...
    combined_data = {}  
    combined_data['tunnel 12 pre-change']={}

    for device, data in device_facts.items():
        try:
            combined_data['tunnel 12 pre-change'][device]={}
            combined_data['tunnel 12 pre-change'][device]['psk'] = crypto_config[device]['keys'][ld6_public_ip]
            combined_data['tunnel 12 pre-change'][device]['profile'] = spoke_tunnels_parsed[device]['Tunnel12']['profile']
            combined_data['tunnel 12 pre-change'][device]['tunnel state'] = dmvpn[device]['Tunnels']['12'][ld6_public_ip]['state']
            if str(dmvpn[device]['Tunnels']['12'][ld6_public_ip]['tunnel IP']) in eigrp_neighbours[device]['neighbours'].keys():
                combined_data['tunnel 12 pre-change'][device]['tunnel 12 nbr'] = str(dmvpn[device]['Tunnels']['12'][ld6_public_ip]['tunnel IP'])
            else:
                ...
                combined_data['tunnel 12 pre-change'][device]['tunnel 12 nbr'] = '[red]not found[/red]'
                #combined_data['tunnel 12 pre-change'].pop(device)
                #continue
    ...
    for device, data in combined_data['tunnel 12 pre-change'].items():
        update_required = False
        if combined_data['tunnel 12 pre-change'][device]['tunnel state'] == 'UP':
            combined_data['tunnel 12 pre-change'][device]['tu12state'] = '[green]\u2713[/green]'
            tu12state = '[green]\u2713[/green]'
        else:
            combined_data['tunnel 12 pre-change'][device]['tu12state'] = '[red]\u2717[/red]'
            tu12state = '[red]\u2717[/red]'
        if combined_data['tunnel 12 pre-change'][device]['psk'] == new_psk:
            combined_data['tunnel 12 pre-change'][device]['psk_match'] = '[green]\u2713 New PSK[/green]'
            psk_match = '[green]\u2713 New PSK[/green]'
        else:
            combined_data['tunnel 12 pre-change'][device]['psk_match'] = '[red]\u2717 old PSK[/red]'
            psk_match = '[red]\u2717 old PSK[/red]'
            update_required = True
        if combined_data['tunnel 12 pre-change'][device]['tunnel 12 nbr'] == ld6_tunnel_ip:
            eigrp_neighbour = '[green]\u2713[/green]'
            combined_data['tunnel 12 pre-change'][device]['tunnel 12 nbr match'] = '[green]\u2713[/green]'
        else:
            eigrp_neighbour = '[red]\u2717[/red]'
            combined_data['tunnel 12 pre-change'][device]['tunnel 12 nbr match'] = '[red]\u2717[/red]'
        if combined_data['tunnel 12 pre-change'][device]['profile'] == old_profile['name']:
            combined_data['tunnel 12 pre-change'][device]['profile_match'] = '[green]\u2713 Old profile[/green]'
            profile_match = '[green]\u2713 Old profile[/green]'
        else:
            combined_data['tunnel 12 pre-change'][device]['profile_match'] = '[red]\u2717 New profile[/red]'
            profile_match = '[red]\u2717 New profile[/red]'
            update_required = True
        if update_required == True:
            combined_data['tunnel 12 pre-change'][device]['update'] = '[yellow]\u2713[/yellow]'
        ...
        dmvpnpskpretable.add_row(device,tu12state,eigrp_neighbour,profile_match,psk_match,update)
    console.print(dmvpnpskpretable)
    ...
    with open('/dbdev/retail_dmvpn_cipher/psk_config_rollback_ld6.txt') as f:
        psk_config = f.read().splitlines()
    ...
    for device in devices_to_configure:
        config_changes[device]=psk_config
    ...
    ################### WRITE DICT TO JSON #####################
        ...
        filepath = '/dbdev/retail_dmvpn_cipher/outputs/site_configs/'+device+'/psk_config_changes_latest.txt'
        with open(filepath, "w") as outfile: 
    ...
    ########### ADD DATA TO LOGS ##################
        ...
        logfilename = 'psk_config_changes_' + curtime + '.txt'
    ...
    filepath4 = '/dbdev/retail_dmvpn_cipher/outputs/results/'+curfoltime+'/psk_config_changes.json'
    ...
    #psk_update=cfg_devices.run(task=update_cypher_config)
    #clean config results
    psk_update_clean,cipher_update_failed_hosts = rhf.clean_facts(psk_update)

    


    #print('####################testing function#####################')
    processed_results,full_processed_results = rhf.process_update_results(psk_update_clean)
    #print('#########################################')
    ...
    for device, data in combined_data['tunnel 12 post-change'].items():
        update_required = False
        if combined_data['tunnel 12 post-change'][device]['tunnel 12 state'] == 'UP':
            combined_data['tunnel 12 post-change'][device]['tu12state'] = '[green]\u2713[/green]'
            tu12state = '[green]\u2713[/green]'
        else:
            combined_data['tunnel 12 post-change'][device]['tu12state'] = '[red]\u2717[/red]'
            tu12state = '[red]\u2717[/red]'
        if combined_data['tunnel 12 post-change'][device]['psk'] == new_psk:
            combined_data['tunnel 12 post-change'][device]['psk_match'] = '[green]\u2713 New PSK[/green]'
            psk_match = '[green]\u2713 New PSK[/green]'
        else:
            combined_data['tunnel 12 post-change'][device]['psk_match'] = '[red]\u2717 old PSK[/red]'
            psk_match = '[red]\u2717 old PSK[/red]'
            update_required = True
        if combined_data['tunnel 12 post-change'][device]['tunnel 12 nbr'] == ld6_tunnel_ip:
            eigrp_neighbour = '[green]\u2713[/green]'
            combined_data['tunnel 12 post-change'][device]['tunnel 12 nbr match'] = '[green]\u2713[/green]'
        else:
            eigrp_neighbour = '[red]\u2717[/red]'
            combined_data['tunnel 12 post-change'][device]['tunnel 12 nbr match'] = '[red]\u2717[/red]'
        if combined_data['tunnel 12 post-change'][device]['profile'] == old_profile['name']:
            combined_data['tunnel 12 post-change'][device]['profile_match'] = '[green]\u2713 Old profile[/green]'
            profile_match = '[green]\u2713 Old profile[/green]'
        else:
            combined_data['tunnel 12 post-change'][device]['profile_match'] = '[red]\u2717 New profile[/red]'
            profile_match = '[red]\u2717 New profile[/red]'
            update_required = True
        if update_required == True:
            combined_data['tunnel 12 post-change'][device]['update'] = '[yellow]\u2713[/yellow]'
            update = '[yellow]\u2713[/yellow]'
            devices_to_configure.append(device)
        elif update_required == False:
            combined_data['tunnel 12 post-change'][device]['update'] = '[yellow]\u2717[/yellow]'
            update = '[yellow]\u2717[/yellow]'
            devices_configured.append(device)
        dmvpnpskposttable.add_row(device,tu12state,eigrp_neighbour,profile_match,psk_match,update)
    console.print(dmvpnpskposttable)
    console.print('''Key: Configured correctly - [green]\u2713[/green]
     Update required - [yellow]\u2713[/yellow]
     Not configured - [yellow]\u271B[/yellow]
     Update not required - [yellow]\u2717[/yellow]
    ...
    for device, data in combined_data['tunnel 12 post-change'].items():
        dmvpnpskresultstable.add_row(device,str(combined_data['tunnel 12 pre-change'][device]['tu12state'])+'[yellow]     |     [/yellow]'+str(combined_data['tunnel 12 post-change'][device]['tu12state']),str(combined_data['tunnel 12 pre-change'][device]['tunnel 12 nbr match'])+'[yellow]     |     [/yellow]'+str(combined_data['tunnel 12 post-change'][device]['tunnel 12 nbr match']),str(combined_data['tunnel 12 pre-change'][device]['profile_match'])+'[yellow]  |  [/yellow]'+str(combined_data['tunnel 12 post-change'][device]['profile_match']),str(combined_data['tunnel 12 pre-change'][device]['psk_match'])+'[yellow]  |  [/yellow]'+str(combined_data['tunnel 12 post-change'][device]['psk_match']))
    console.print(dmvpnpskresultstable)


    console.print("[blue]##################\nStep 5 of 5 - Comparing end state with state pre change, please wait\n##################")
    
    #with open(f'/dbdev/retail_dmvpn_cipher/outputs/golden_tunnel_state/ld6_pre_change_state.pickle', 'wb') as file:
    #    dmvpntable = pickle.load(file)
    with open('/dbdev/retail_dmvpn_cipher/outputs/golden_tunnel_state/ld6_pre_change_state.json') as file1:
