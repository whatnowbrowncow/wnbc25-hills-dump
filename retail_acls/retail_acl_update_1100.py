...
console = Console()
# Local artefacts
import retail_helper_functions_1100 as rhf
...
new_acl_numbers = ['102','103','121','122','123','141']
...
new_syntax_acls = {
    '102':
['ip access-list extended 102',' remark Allow ICMP',' permit icmp any any',' remark Remote Store Subnets',' deny   ip any 10.160.0.0 0.7.255.255',' deny   ip any 10.93.0.0 0.0.255.255',' deny   ip any 10.94.0.0 0.0.255.255',' deny   ip any 10.95.0.0 0.0.255.255',' deny   ip any 10.96.0.0 0.0.255.255',' permit ip any any'],
    '103':{'pre':
['ip access-list extended 103',' remark Allow ICMP',' permit icmp any any',' remark Allow Local 120 Subnet'],
           'post':
[' remark Remote Store Subnets',' deny   ip any 10.160.0.0 0.7.255.255',' deny   ip any 10.93.0.0 0.0.255.255',' deny   ip any 10.94.0.0 0.0.255.255',' deny   ip any 10.95.0.0 0.0.255.255',' deny   ip any 10.96.0.0 0.0.255.255',' permit ip any any']
},
    '121':{'pre':
['ip access-list extended 121',' remark Allow ICMP',' permit icmp any any',' remark Allow Local 120 and 130 Subnets'],
           'post':
[' remark Remote Store Subnets',' deny   ip any 10.160.0.0 0.7.255.255',' deny   ip any 10.93.0.0 0.0.255.255',' deny   ip any 10.94.0.0 0.0.255.255',' deny   ip any 10.95.0.0 0.0.255.255',' deny   ip any 10.96.0.0 0.0.255.255',' permit ip any any']
},
    '122':{'pre':
['ip access-list extended 122',' remark Allow ICMP',' permit icmp any any',' remark Allow Local 1 and 130 Subnets'],
           'post':
[' remark Remote Store Subnets',' deny   ip any 10.160.0.0 0.7.255.255',' deny   ip any 10.93.0.0 0.0.255.255',' deny   ip any 10.94.0.0 0.0.255.255',' deny   ip any 10.95.0.0 0.0.255.255',' deny   ip any 10.96.0.0 0.0.255.255',' permit ip any any']
},
    '123':{'pre':
['ip access-list extended 123',' remark Allow ICMP',' permit icmp any any',' remark Allow Local 120 Subnet'],
           'post':
[' remark Remote Store Subnets',' deny   ip any 10.160.0.0 0.7.255.255',' deny   ip any 10.93.0.0 0.0.255.255',' deny   ip any 10.94.0.0 0.0.255.255',' deny   ip any 10.95.0.0 0.0.255.255',' deny   ip any 10.96.0.0 0.0.255.255',' permit ip any any']
},
    '141':
['ip access-list extended 141',' remark Allow ICMP',' permit icmp any any',' remark Store to Store Voip Calls',' permit udp any 10.163.0.0 0.0.255.255 range 16384 32767',' remark Remote Store Subnets',' deny   ip any 10.160.0.0 0.7.255.255',' deny   ip any 10.93.0.0 0.0.255.255',' deny   ip any 10.94.0.0 0.0.255.255',' deny   ip any 10.95.0.0 0.0.255.255',' deny   ip any 10.96.0.0 0.0.255.255',' permit ip any any']   
}
...
#
    ...
    numbered_acls_parsed,new_acl_devices,old_acl_devices = rhf.get_numbered_acls(device_facts,aclnos,new_acl_numbers,4)
    ...
    for device,values in versions_parsed.items():
        ...
        if 'C111' in values['chassis']:
            upgraded_devices.append(device)
...
## create various output files
    ...
    for device,interfaces in combined_data.items():
        ...
        if device in new_acl_devices:
            new_acls = new_syntax_acls
        elif device in old_acl_devices:
            new_acls = old_syntax_acls
                
        for acl in list(new_acls):
            if device in new_acl_devices:
                prefix = ' permit ip '
            elif device in old_acl_devices:
                prefix = str('access-list '+acl+' permit ip ')
            
            if str(acl) == "103":
                temp_103_acl = []
                for line in new_acls[acl]['pre']:
                    temp_103_acl.append(line)
                temp_103_acl.append(str(prefix)+ str(ip10.network)+' '+str(ip10.hostmask) +' '+str(ip120.network)+' '+str(ip120.hostmask))
                ...
                if acl in numbered_acls_parsed[device]['new_acls'].keys():
                    if temp_103_acl == numbered_acls_parsed[device]['new_acls'][acl]:
                        ...
                        config_changes[device].append(str(prefix)+ str(ip10.network)+' '+str(ip10.hostmask) +' '+str(ip120.network)+' '+str(ip120.hostmask))
                        for line in new_acls[acl]['post']:
                            config_changes[device].append(line)
                else:
                    for line in new_acls[acl]['pre']:
                ...
                temp_121_acl.append(str(prefix)+ str(ip120.network)+' '+str(ip120.hostmask) +' '+str(ip130.network)+' '+str(ip130.hostmask))
                temp_121_acl.append(str(prefix)+ str(ip130.network)+' '+str(ip130.hostmask) +' '+str(ip120.network)+' '+str(ip120.hostmask))
                for line in new_acls[acl]['post']:
                    temp_121_acl.append(line)
                if acl in numbered_acls_parsed[device]['new_acls'].keys():
                    if temp_121_acl == numbered_acls_parsed[device]['new_acls'][acl]:
                        ...
                        config_changes[device].append(str(prefix)+ str(ip120.network)+' '+str(ip120.hostmask) +' '+str(ip130.network)+' '+str(ip130.hostmask))
                        config_changes[device].append(str(prefix)+ str(ip130.network)+' '+str(ip130.hostmask) +' '+str(ip120.network)+' '+str(ip120.hostmask))
                        for line in new_acls[acl]['post']:
                            config_changes[device].append(line)
                else:
                    for line in new_acls[acl]['pre']:
                ...
                temp_122_acl.append(str(prefix)+ str(ip120.network)+' '+str(ip120.hostmask) +' '+str(ip10.network)+' '+str(ip10.hostmask))
                temp_122_acl.append(str(prefix)+ str(ip120.network)+' '+str(ip120.hostmask) +' '+str(ip130.network)+' '+str(ip130.hostmask))
                for line in new_acls[acl]['post']:
                    temp_122_acl.append(line)
                if acl in numbered_acls_parsed[device]['new_acls'].keys():
                    if temp_122_acl == numbered_acls_parsed[device]['new_acls'][acl]:
                        ...
                        config_changes[device].append(str(prefix)+ str(ip120.network)+' '+str(ip120.hostmask) +' '+str(ip10.network)+' '+str(ip10.hostmask))
                        config_changes[device].append(str(prefix)+ str(ip120.network)+' '+str(ip120.hostmask) +' '+str(ip130.network)+' '+str(ip130.hostmask))
                        for line in new_acls[acl]['post']:
                            config_changes[device].append(line)
                else:
                ...
                temp_123_acl.append(str(prefix)+ str(ip130.network)+' '+str(ip130.hostmask) +' '+str(ip120.network)+' '+str(ip120.hostmask))
                for line in new_acls[acl]['post']:
                    temp_123_acl.append(line)
                if acl in numbered_acls_parsed[device]['new_acls'].keys():
    ...
    console.print('[red] upgraded devices {}'.format(str(len(new_acl_devices))))
    #for device in new_acl_devices:
    #    print(device)
    console.print('[red] Old style devices {}'.format(str(len(old_acl_devices))))
    #for device in old_acl_devices:
    #    print(device)
    trunk_devices = []
    for device,interfaces in combined_data.items():
        if len(list(interfaces)) == 0:
            trunk_devices.append(device)
            #console.print('[red]{} appears to have no sub interfaces'.format(device))



    console.print('[bold blue]################ SUMMARY OF CHANGES NEEDED #################')
    ...
    for device in target_devices:
        console.print('[blue]- {}'.format(device))
    console.print("Total devices requiring hostname change: [blue]{}".format(len(list(hostname_change_required))))
    console.print("Total devices without sub interfaces configured: [red]{}".format(len(trunk_devices)))
    for device in trunk_devices:
        console.print('[red]- {}'.format(device))
