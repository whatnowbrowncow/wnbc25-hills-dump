...
if __name__ == "__main__":
    ...
    console.print("[blue]##################\nStep 1 of 7 - Gathering device facts ({} devices), this may take a while\n##################".format(len(nr_devices.inventory.hosts)))
    with tqdm(
...
#
    ...
    console.print("[blue]##################\nStep 2 of 7 - Checking for hostname miss-matches\n##################")
    ...
    console.print("[blue]##################\nStep 3 of 7 - Checking for any device errors\n##################")
...
## create various output files
    filepath = '/dbdev/retail_acls/outputs/sub_acls_parsed.json'
    with open(filepath, "w") as outfile: 
        json.dump(sub_acls_parsed, outfile)
    filepath1 = '/dbdev/retail_acls/outputs/combined_data.json'
    with open(filepath1, "w") as outfile: 
        json.dump(combined_data, outfile)
    filepath2 = '/dbdev/retail_acls/outputs/failed_hosts.json'
    with open(filepath2, "w") as outfile: 
        json.dump(failed_hosts, outfile)
    filepath3 = '/dbdev/retail_acls/outputs/numbered_acls_parsed.json'
...
## compare the ACLs configured on the devices to the 'golden config'
    console.print("[italic blue]##################\nStep 4 of 7 - running ACL comparisons\n#################")
#
    for device,values in combined_data.items():
            ...
            for interface in values.keys():
                if values[interface]['acl'] != 'none configured' and acl in values[interface]['acl'].keys() and values[interface]['vlan'] in golden_config['acls'][acl]['vlans']:
                    ...
                    if values[interface]['acl'][str(acl+'-acl')] == golden_config['acls'][acl]['acl']:
                        match_results.append(str(acl)+' : [green]MATCH[/green] | ')
                        #console.print('ACL {}++++++++++++ [green]MATCH'.format(str(acl)))
                    else:
                        #console.print('ACL {}++++++++++++ [red]FAIL'.format(str(acl)))
                        match_results.append(str(acl)+' : [red]FAIL[/red] | ')
                        #d = difflib.Differ()
                        #diff = list(d.compare(golden_config['acls'][acl]['acl'].splitlines(),values[interface]['acl'][str(acl+'-acl')].splitlines()))
        console.print('[italic blue]{}[/italic blue] | '.format(str(device))+''.join(match_results))
                        #pprint(diff)
                    #except Exception as e:
                        #print(e)
                #else:
        ...
        #console.print("[blue]------------------------------------------------------")
    console.print("[italic blue]##################\nStep 5 of 7 - Processing unique ACLs\n###############")
    unique_acls={}
    for acl in golden_config['acls'].keys():
        device_count = 0
        unique_acls[acl]={}
...
#
    ...
    filepath = '/dbdev/retail_acls/outputs/unique_acls.json'
    with open(filepath, "w") as outfile: 
    ...
    target_devices = []
    skipped_devices = []
    console.print("[italic blue]##################\nStep 6 of 7 - generating config changes for each device\n#####################")
    ...
    filepath4 = '/dbdev/retail_acls/outputs/config_changes.json'
    with open(filepath4, "w") as outfile: 
        json.dump(config_changes, outfile)
    filepath5 = '/dbdev/retail_acls/outputs/config_rollback.json'
    with open(filepath5, "w") as outfile: 
        json.dump(config_rollback, outfile)

###############################unique configs####################################
    console.print("[blue]##################\nStep 7 of 7 - Processing unique configs\n##################")
    unique_configs={}
    configs = 0
    ...
    filepath6 = '/dbdev/retail_acls/outputs/unique_configs.json'
...
#
    ...
    filepath10 = '/dbdev/retail_acls/outputs/failed_config_hosts.json'
    ...
    filepath7 = '/dbdev/retail_acls/outputs/processed_results.json'
    with open(filepath7, "w") as outfile: 
        json.dump(processed_results, outfile)
    #save it
    with open(f'/dbdev/retail_acls/full_processed_results.pickle', 'wb') as file:
