...
## create various output files
    ...
    for device,interfaces in combined_data.items():
        for interface,values in interfaces.items():
            if '.120' in str(interface) or '.1020' in str(interface):
        ...
        if device in new_acl_devices:
            ...
            prefix = ' permit ip '
        if device in old_acl_devices:
            new_acls = old_syntax_acls
            prefix = 'access-list 121 permit ip '
        ...
        #print(str('#########'+device+'########'))
        for interface,values in interfaces.items():
            if '.120' not in str(interface) and '.1020' not in str(interface) and '.130' not in str(interface) and '.1030' not in str(interface) and '.140' not in str(interface) and '.1040' not in str(interface):
                ...
                if str(list(combined_data[device][interface]['acl'])[0]) != "141":
                    ...
                    if combined_data[device][interface]['acl'] == 'none configured':
                        ...
                        config_rollback[device].append('interface '+interface)
                        config_rollback[device].append('ip access-group '+str(list(combined_data[device][interface]['acl'])[0])+' in')
            elif '.120' in str(interface) or '.1020' in str(interface) or '.130' in str(interface) or '.1030' in str(interface):
                if str(list(combined_data[device][interface]['acl'])[0]) != "121":
                    config_changes[device].append('interface '+interface)
                    config_changes[device].append('ip access-group 121 in')
                    if combined_data[device][interface]['acl'] == 'none configured':
                        config_rollback[device].append('interface '+interface)
                        config_rollback[device].append('no ip access-group 121 in')
                    else:
                        config_rollback[device].append('interface '+interface)
                        config_rollback[device].append('ip access-group '+str(list(combined_data[device][interface]['acl'])[0])+' in')  
    ...
    console.print('[red] upgraded devices')
    for device in upgraded_devices:
        print(device)
    exit()
