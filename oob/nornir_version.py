...
console = Console()
# Local artefacts
import oob.oob_helper_functions as rhf
...
if __name__ == "__main__":
    ...
    versions = nr_devices.run(task=netmiko_send_command, command_string="show version", use_genie=True, use_timing=True)
    for device,details in versions.items():
    ...
    versions_parsed = rhf.get_version(versions)
    print(versions_parsed)

    device_versions = {}
    for host,data in versions_parsed.items():
        if data['router_type'] not in device_versions.keys():
            device_versions[data['router_type']]=[]
            device_versions[data['router_type']].append(host)
        else:
            device_versions[data['router_type']].append(host)

    print(device_versions)

    filepath = './retail_versions.json'
    with open(filepath, "w") as outfile: 
        json.dump(device_versions, outfile)
