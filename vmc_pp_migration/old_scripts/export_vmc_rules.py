...
#    if group["_create_user"]!= "admin" and group["_create_user"]!="admin;admin":
#        core_gw_groups[str(group["display_name"])] = {}
#        if "description" in core_gw_groups[str(group["display_name"])].keys():
#            core_gw_groups[str(group["display_name"])]["description"] = group["description"]
#        else:
#            core_gw_groups[str(group["display_name"])]["description"] = "No description configured"
#        core_gw_groups[str(group["display_name"])]["created by"] = group["_create_user"]
#        core_gw_groups[str(group["display_name"])]["object type"] = group["expression"][0]["resource_type"]
#        if group["expression"][0]["resource_type"] == "IPAddressExpression":
#             core_gw_groups[str(group["display_name"])]["IP Addresses"] = group["expression"][0]["ip_addresses"]
#print(core_gw_groups)
#        #print(json.dumps(group,indent=4))
#
...
for cmap in cmaps:
    ...
    s = json.loads(dfwsecresponse.text)
    print(s)
    sections =s["results"]
